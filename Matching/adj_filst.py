...
np.random.seed(142)
cps_recs['z1'] = cps_recs['case1'].apply(lambda x: np.random.uniform(0, 1) if x == 1 else x)
np.random.seed(1)
cps_recs['z2'] = cps_recs['case2'].apply(lambda x: np.random.uniform(0, 1) if x == 1 else x)

selected = (cps_recs['z1'] <= 0.84) | (cps_recs['z2'] <= 0.54)  #TODO: check the probability
